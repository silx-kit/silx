...
- Sort, curate and fix the list of PRs and match the styling of previous release notes. 
  You can save them in a file and run ``tools/format_GH_release_notes.py <name_of_the_file>`` to produced formatted
  release notes.
...
- Update ``CHANGELOG.rst`` with the changes from the release, generated in the previous section.
...
* Draft a new release from `github new release page`_ using similar conventions as previous releases:
  ...
  - Copy the relevant section of the ``CHANGELOG.rst`` in the release notes.
